      ...
      indexOf(t, r = null, n = 'utf-8', i = !1) {
        ...
        return (
          ...
          s.length === 0 && i === !1
            ...
            : o(
                (f, T) =>
                  (i ? T <= a : T >= a) && this[T] === s[0] && s.every((A, R) => this[T + R] === A)
              )
        );
      }
...
function tp(e, t) {
  ...
  n && (n.removeAllFields(), Ns(n, r)),
    t.addErrorMessage(o =>
      i
        ? `The ${o.red('`select`')} statement for type ${o.bold(r.name)} must not be empty. ${rr(
            o
          )}`
...
function bd(e, t) {
  switch (t.error_identifier) {
    ...
    case 'INCOMPLETE_CONNECT_INPUT':
      ...
      } records to be connected, found only ${Array.isArray(e) ? e.length : e}.`;
    case 'INCOMPLETE_CONNECT_OUTPUT':
      return `The required connected records were not found. Expected ${
        t.context.expectedRows
      } records to be connected after connect operation on ${t.context.relationType} relation '${
        t.context.relation
      }', found ${Array.isArray(e) ? e.length : e}.`;
    ...
    async withRetry(t) {
      for (let r = 0; ; r++) {
        ...
        try {
          ...
          this.logEmitter.emit('warn', {
            message: `Attempt ${r + 1}/${pu} failed for ${t.actionGerund}: ${
              i.message ?? '(unknown)'
            }`,
            timestamp: new Date(),
            target: '',
